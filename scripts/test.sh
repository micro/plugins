...
for d in $(find $version -name 'go.mod'); do
  case $(dirname $d) in
    ...
    echo SKIP $(dirname $d)
    ;;
    'v4/config/source/configmap'|'v4/config/source/vault'|'v4/events/redis'|'v4/events/natsjs'|'v4/logger/windowseventlog'|'v4/store/mysql'|'v4/store/redis'|'v4/sync/consul'|'v4/sync/etcd'|'v4/cache/redis')
    echo SKIP $(dirname $d)
    ;;
    *)
