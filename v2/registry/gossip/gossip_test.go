...
func TestGossipRegistryBroadcast(t *testing.T) {
	...
	if !found {
		t.Errorf("[gossip registry] service.2 not found in r1, broadcast not work")
	}
	...
	if !found {
		t.Errorf("[gossip registry] broadcast failed: service.1 not found in r2")
	}
...
func TestGossipRegistryRetry(t *testing.T) {
	...
	if err = r1.(*gossipRegistry).Stop(); err != nil {
		t.Errorf("[gossip registry] failed to stop registry: %v", err)
	}
	...
	if found {
		t.Errorf("[gossip registry] service.1 found in r2")
	}
	...
	if !found {
		t.Errorf("[gossip registry] connect retry failed: service.1 not found in r2")
	}
