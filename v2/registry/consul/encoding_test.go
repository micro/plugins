...
func TestEncodingEndpoints(t *testing.T) {
	...
	testEp := func(ep *registry.Endpoint, enc string) {
		...
		if len(e) != 1 {
			t.Errorf("Expected 1 encoded tags, got %v", e)
		}
		...
		if !seen {
			t.Errorf("Expected %s but not found", enc)
		}

		// decode
		d := decodeEndpoints([]string{enc})
		if len(d) == 0 {
			t.Errorf("Expected %v got %v", ep, d)
		}

		// check name
		if d[0].Name != ep.Name {
			t.Errorf("Expected ep %s got %s", ep.Name, d[0].Name)
		}

		// check all the metadata exists
		for k, v := range ep.Metadata {
			if gv := d[0].Metadata[k]; gv != v {
				t.Errorf("Expected key %s val %s got val %s", k, v, gv)
			}
		}
	}
...
func TestEncodingVersion(t *testing.T) {
	...
	for _, data := range testData {
		...
		if e[0] != data.encoded {
			t.Errorf("Expected %s got %s", data.encoded, e)
		}

		d, ok := decodeVersion(e)
		if !ok {
			t.Errorf("Unexpected %t for %s", ok, data.encoded)
		}

		if d != data.decoded {
			t.Errorf("Expected %s got %s", data.decoded, d)
		}
