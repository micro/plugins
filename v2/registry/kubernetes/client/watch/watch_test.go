...
func TestBodyWatcher(t *testing.T) {
	...
	if err != nil {
		t.Errorf("did not expect NewRequest to return err: %v", err)
	}

	// setup body watcher
	w, err := NewBodyWatcher(req, http.DefaultClient)
	if err != nil {
		t.Errorf("did not expect NewBodyWatcher to return %v", err)
	}
